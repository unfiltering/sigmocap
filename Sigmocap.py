    ...
    frame_data = {
        'joints': {
            bone_mapping[idx]: {
                'x': float(joint[0]),
                'y': float(joint[1]),
                'z': float(joints[idx][2]) if len(joints[idx]) > 2 else None
            }
    ...
    if left_hand_landmarks:
        frame_data['left_hand'] = {
            f'left_hand_joint_{idx}': {
                'x': float(landmark.x),
                'y': float(landmark.y),
                'z': float(landmark.z) if hasattr(landmark, 'z') else None
...
def save_recording(recorded_data):
    ...
    json_data = json.dumps(recorded_data)
    base64_data = base64.b64encode(json_data.encode()).decode()
    encrypted_data = custom_encrypt(base64_data.encode())
...
while cap.isOpened():
    try:
        ...
        if cv2.getWindowProperty("SigMocap", cv2.WND_PROP_VISIBLE) < 1:
            ...
            files_to_send = [log_file_path, file_path]
